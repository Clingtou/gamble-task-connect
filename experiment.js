...
const CSV_COLUMNS = [
  ...
  "PostTaskChoiceInfluence", "PostTaskChoiceInfluenceOther", "PostTaskFamiliarity",
...
let postTaskResponses = {
  amountReadingOrder: "",
  amountReadingOrderOther: "",
  fontReadingOrder: "",
  fontReadingOrderOther: "",
  fontPattern: "",
  fontPatternOther: "",
  choiceInfluence: "",
  choiceInfluenceOther: "",
  fontSizeRating: "",
  familiarity: "",
  decisionStrategy: "",
  strategyWordCount: 0,
...
function isConnectCompletionConfigured() {
  return Boolean(CONNECT_COMPLETION_CODE.trim() && CONNECT_COMPLETION_URL.trim());
}
...
function prepareResults() {
  results = trials.map((trial, index) => ({
    ...
    PostTaskAmountReadingOrder: "",
    PostTaskAmountReadingOrderOther: "",
    PostTaskFontReadingOrder: "",
    PostTaskFontReadingOrderOther: "",
    PostTaskFontPattern: "",
    PostTaskFontPatternOther: "",
    PostTaskChoiceInfluence: "",
    PostTaskChoiceInfluenceOther: "",
    PostTaskFamiliarity: "",
...
function postTaskChoiceQuestion(name, title, options, allowOther = true) {
  const choices = allowOther ? [...options, ["other", "Other (please specify)"]] : options;
  return `<div class="form-question">
    <div id="${name}-question" class="question-text">${title}</div>
    <div class="single-choice-list" role="radiogroup" aria-labelledby="${name}-question" aria-describedby="${name}-required" aria-required="true">
      ${choices.map(([value, label]) => `<label class="single-choice-option"><input type="radio" name="${name}" value="${value}" required><span>${label}</span></label>`).join("")}
    </div>
    ${allowOther ? `<input id="${name}-other" type="text" class="post-task-other" placeholder="Please specify" aria-label="${title}: other, please specify" disabled>` : ""}
    <div id="${name}-required" class="question-required" role="alert">Please select one option. If you select “Other,” please provide details.</div>
  </div>`;
}

async function postTaskQuestionsAndWait() {
  const amountReadingOrder = [["gain_first", "Usually looked at the gain first, then the loss"], ["loss_first", "Usually looked at the loss first, then the gain"], ["simultaneous", "Usually looked at both about the same time"], ["no_fixed_order", "No consistent order"], ["indistinguishable", "Hard to tell the order"]];
  const fontReadingOrder = [["large_first", "Usually looked at the larger font first, then the smaller font"], ["small_first", "Usually looked at the smaller font first, then the larger font"], ["simultaneous", "Usually looked at both about the same time"], ["no_fixed_order", "No consistent order"], ["indistinguishable", "Hard to tell the order"]];
  const fontPattern = [["gain", "Gains"], ["loss", "Losses"], ["alternating", "Sometimes gains, sometimes losses"], ["not_sure", "Didn’t notice / not sure"]];
  const choiceInfluence = [["none", "No, I did not feel that it affected my decisions."], ["accept", "Yes, it generally made me more inclined to accept the gambles."], ["reject", "Yes, it generally made me more inclined to reject the gambles."], ["unclear_direction", "Yes, but I cannot tell whether it made me more inclined to accept or reject."], ["not_sure", "Not sure / can’t remember."]];
  const familiarity = [["no", "No"], ["yes_once", "Yes, once"], ["yes_more_than_once", "Yes, more than once"], ["not_sure", "Not sure"]];
  const pages = [
    `<div class="form-question">
        <div id="font-size-question" class="question-text">1. In the gamble task, to what extent did amounts presented in a larger font give you the impression that their numerical magnitudes were also larger?</div>
        <p id="font-size-scale-description">(1 = not at all, 7 = very strongly)</p>
        <div class="post-task-scale" role="radiogroup" aria-labelledby="font-size-question" aria-describedby="font-size-scale-description font-size-required" aria-required="true">
          <div class="post-task-scale-anchors" aria-hidden="true"><span>Not at all</span><span>Very strongly</span></div>
          <div class="post-task-scale-options">
            ${[1, 2, 3, 4, 5, 6, 7].map((rating) => `
              <label class="post-task-scale-option">
                <input type="radio" name="font_size_rating" value="${rating}" required aria-label="${rating}${rating === 1 ? ' — Not at all' : rating === 7 ? ' — Very strongly' : ''}">
                <span>${rating}</span>
              </label>
            `).join("")}
          </div>
      ...
      ${postTaskChoiceQuestion("amountReadingOrder", "2. Which of the following best describes how you typically looked at the gain and loss amounts?", amountReadingOrder)}
      ${postTaskChoiceQuestion("fontReadingOrder", "3. Thinking specifically about the font size of the two amounts, which of the following best describes your typical viewing order?", fontReadingOrder)}`,
    `${postTaskChoiceQuestion("fontPattern", "1. Which amounts were shown in the larger font during the task?", fontPattern)}
      ${postTaskChoiceQuestion("choiceInfluence", "2. Did you feel that your decisions in the gamble task were influenced by the relative font size of the two amounts?", choiceInfluence)}
      ${postTaskChoiceQuestion("familiarity", "3. Before today, had you ever completed a similar decision-making task involving gambles?", familiarity, false)}
      <div class="form-question">
  ...
  const pageFields = [["amountReadingOrder", "fontReadingOrder"], ["fontPattern", "choiceInfluence", "familiarity"]];
  for (let page = 0; page < pages.length; page += 1) {
    if (aborted) return false;
    showContent(`<p class="post-task-heading"><strong>Now, please answer a few questions about your decisions.</strong></p>
      <form id="post-task-form" novalidate>${pages[page]}
      <button id="post-task-submit" class="content-button" type="submit">${page === 0 ? "Next" : "Submit and view result"}</button></form>`, "post-task-page");
    ...
    const captureResponses = () => {
      for (const name of pageFields[page]) {
        const selected = form.querySelector(`input[name="${name}"]:checked`);
        postTaskResponses[name] = selected ? selected.value : "";
        const other = document.getElementById(`${name}-other`);
        if (other) {
          other.disabled = postTaskResponses[name] !== "other";
          postTaskResponses[`${name}Other`] = other.disabled ? "" : other.value.trim();
        }
      }
      if (page === 0) {
        const rating = form.querySelector('input[name="font_size_rating"]:checked');
        postTaskResponses.fontSizeRating = rating ? Number(rating.value) : "";
      }
      if (strategyInput) {
        ...
        document.getElementById("strategy-word-count").textContent = `${postTaskResponses.strategyWordCount} words (minimum: ${MIN_STRATEGY_WORDS})`;
      }
    };
    form.addEventListener("input", captureResponses);
    form.addEventListener("change", captureResponses);
    await new Promise((resolve) => {
      activeWait = { allowedCodes: [], resolve };
      form.addEventListener("submit", (event) => {
        event.preventDefault();
        if (aborted || phase !== "post_task_questions") return;
        captureResponses();
        let invalidInput = null;
        if (page === 0) {
          const valid = Number.isInteger(postTaskResponses.fontSizeRating) && postTaskResponses.fontSizeRating >= 1 && postTaskResponses.fontSizeRating <= 7;
          document.getElementById("font-size-required").style.display = valid ? "none" : "block";
          if (!valid) invalidInput = form.querySelector('input[name="font_size_rating"]');
        }
        for (const name of pageFields[page]) {
          const selected = postTaskResponses[name];
          const valid = Boolean(selected) && (selected !== "other" || Boolean(postTaskResponses[`${name}Other`]));
          document.getElementById(`${name}-required`).style.display = valid ? "none" : "block";
          if (!valid && !invalidInput) invalidInput = selected === "other" ? document.getElementById(`${name}-other`) : form.querySelector(`input[name="${name}"]`);
        }
        if (strategyInput) {
          const valid = postTaskResponses.strategyWordCount >= MIN_STRATEGY_WORDS;
          document.getElementById("strategy-required").style.display = valid ? "none" : "block";
          strategyInput.setAttribute("aria-invalid", String(!valid));
          if (!valid && !invalidInput) invalidInput = strategyInput;
        }
        if (invalidInput) {
          invalidInput.focus();
          return;
        }
        document.getElementById("post-task-submit").disabled = true;
        activeWait = null;
        phase = page === 0 ? "post_task_page_transition" : "post_task_submitted";
        if (page === 1) postTaskResponses.completed = true;
        resolve();
      });
    });
  }
...
function drawPaymentResult() {
  if (!trials.length || results.length !== trials.length || results.some((row) => ![0, 1].includes(row.Choice))) {
    throw new Error("All formal trials must be completed before payment can be determined.");
  }
  // Select a formal trial uniformly, independently of its amounts and response.
  const randomValue = new Uint32Array(1);
  const randomLimit = Math.floor(4294967296 / trials.length) * trials.length;
  do {
    crypto.getRandomValues(randomValue);
  } while (randomValue[0] >= randomLimit);
  const selectedIndex = randomValue[0] % trials.length;
...
function applySummaryToResults(status) {
  ...
  results.forEach((row, index) => {
    ...
    row.PostTaskAmountReadingOrder = postTaskResponses.amountReadingOrder;
    row.PostTaskAmountReadingOrderOther = postTaskResponses.amountReadingOrderOther;
    row.PostTaskFontReadingOrder = postTaskResponses.fontReadingOrder;
    row.PostTaskFontReadingOrderOther = postTaskResponses.fontReadingOrderOther;
    row.PostTaskFontPattern = postTaskResponses.fontPattern;
    row.PostTaskFontPatternOther = postTaskResponses.fontPatternOther;
    row.PostTaskChoiceInfluence = postTaskResponses.choiceInfluence;
    row.PostTaskChoiceInfluenceOther = postTaskResponses.choiceInfluenceOther;
...
function summaryOnlyRow(status) {
  return {
    ...
    PostTaskAmountReadingOrder: postTaskResponses.amountReadingOrder,
    PostTaskAmountReadingOrderOther: postTaskResponses.amountReadingOrderOther,
    PostTaskFontReadingOrder: postTaskResponses.fontReadingOrder,
    PostTaskFontReadingOrderOther: postTaskResponses.fontReadingOrderOther,
    PostTaskFontPattern: postTaskResponses.fontPattern,
    PostTaskFontPatternOther: postTaskResponses.fontPatternOther,
    PostTaskChoiceInfluence: postTaskResponses.choiceInfluence,
    PostTaskChoiceInfluenceOther: postTaskResponses.choiceInfluenceOther,
...
function finishStudy() {
  ...
  const connectCompletionConfigured = isConnectCompletionConfigured();
  ...
  `, "end-page");
  phase = "finished";
  if (connectCompletionConfigured) {
...
function handleUnexpectedError(error) {
  ...
  showContent("<h1>The study has ended.</h1><div class=\"termination-warning\"><strong>The experiment stopped because of an unexpected error.</strong></div><p>Your data will not be saved because the study was not completed.</p>", "end-page");
  phase = "terminated";
}
